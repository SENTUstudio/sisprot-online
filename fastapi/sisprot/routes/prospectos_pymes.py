...
from sisprot.models import ProspectosPymes, PlantillaMensaje
from sisprot.schemas import ProspectoPymesInSchema
...
@router.get("/prospectos-pymes", dependencies=[Depends(jwt_required)], tags=["prospectos-pymes"])
...
@router.put(
    "/prospectos-pymes/{id}", dependencies=[Depends(jwt_required)], tags=["prospectos-pymes"]
)
def edit(id: int, data: ProspectoPymesInSchema, session=Depends(get_db)):
    data = data.dict(exclude_none=True)
    item = session.query(ProspectosPymes).filter_by(id=id).first()
...
@router.post("/prospectos-pymes", tags=["prospectos-pymes"])
def create(data: ProspectoPymesInSchema, session=Depends(get_db)):
    """
    ...
    item = ProspectosPymes(**data.dict())
